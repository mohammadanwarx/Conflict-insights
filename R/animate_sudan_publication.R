# Sudan Conflict Animation - Publication Quality
# Purpose: Create LinkedIn-optimized animated visualization
# Output: output/sudan_conflict_publication.mp4 (1200x1200px)
#
# Panel Layout:
...
TARGET_FRAMES <- FPS * TARGET_DURATION  # 108 frames
...
#===================================
# Load Sudan boundaries
#===================================
sudan_gpkg_path <- "data/raw/sudan_adm2.gpkg"

if (!file.exists(sudan_gpkg_path)) {
  cat("Downloading Sudan ADM2 boundaries...\n")
  sudan_adm2_raw <- gadm(country = "SDN", level = 2, path = "data/raw")
  sudan_adm2 <- st_as_sf(sudan_adm2_raw)
  st_write(sudan_adm2, sudan_gpkg_path, delete_dsn = TRUE)
} else {
  sudan_adm2 <- st_read(sudan_gpkg_path, quiet = TRUE)
}
cat("Loaded Sudan ADM2 with", nrow(sudan_adm2), "regions\n")

# Get country outline
sudan_outline <- sudan_adm2 %>% st_union()

#===================================
# Download basemap tiles - Sudan extent
#===================================
cat("Downloading basemap tiles...\n")
tiles_cache <- "data/raw/tiles"
if (!dir.exists(tiles_cache)) dir.create(tiles_cache, recursive = TRUE)

# Extended bbox for full canvas coverage (Sudan region)
extended_bbox <- st_bbox(c(
  xmin = 20,
  xmax = 40,
  ymin = 7,
  ymax = 24
), crs = st_crs(sudan_adm2))

extended_area <- st_as_sfc(extended_bbox)

sudan_tiles <- get_tiles(
  x = extended_area,
...
data <- raw_data %>%
  ...
  filter(week >= as.Date("2023-04-01")) %>%
  mutate(
...
cat("Sudan records (2009+, fatalities > 0):", nrow(data), "\n")
...
# Map limits for Sudan
map_xlim <- c(21, 39)
map_ylim <- c(8, 23)

# Pre-render map base layer (tiles + boundaries) once
p_map_base <- ggplot() +
  geom_spatraster_rgb(data = sudan_tiles) +
  geom_sf(data = sudan_adm2, fill = NA, color = "gray50", linewidth = 0.2) +
  geom_sf(data = sudan_outline, fill = NA, color = "#333333", linewidth = 0.8) +
...
map_base_path <- file.path(tempdir(), "sudan_map_base.png")
...
map_base_img <- image_scale(map_base_img, paste0(MAP_WIDTH, "x", MAP_HEIGHT, "!"))
...
create_map_points <- function(state_label_value) {
  ...
  p <- ggplot() +
    ...
    scale_size_continuous(range = c(4, 25), guide = "none") +
...
create_map_title <- function(state_label_value) {
  p <- ggplot() +
    annotate("text", x = 0.4, y = 0.95, label = "Sudan Conflict Fatalities", 
             size = 6, fontface = "bold", color = "#1a1a1a", hjust = 0.5) +
    annotate("text", x = 0.85, y = 0.95, label = state_label_value, 
             size = 6.5, fontface = "bold", color = "#B33A3A", hjust = 1) +
...
temp_dir <- file.path(tempdir(), "sudan_frames")
...
for (i in seq_along(states)) {
  ...
  points_img <- image_scale(points_img, paste0(MAP_WIDTH, "x", MAP_HEIGHT, "!"))
...
output_mp4 <- "output/sudan_conflict_publication.mp4"
...
all_frames <- image_animate(all_frames, fps = 5)  # Use fixed fps that's factor of 100

output_gif <- "output/sudan_conflict_publication.gif"
